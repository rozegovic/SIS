...
{	
	...
	//#endif
		//Body 
	{
		dp::IDataSet* pDP(pDB->createDataSet("select a.ID_Studenta, d.ID_Aktivnosti, b.Naziv_Aktivnosti, c.Indeks, c.Ime, c.Prezime from Prijavljeni_ispiti a, Aktivnosti b, Korisnici c, Rokovi d where d.ID_Aktivnosti = b.ID_Aktivnosti and a.ID_Roka = d.ID_Roka and a.ID_Studenta = c.ID and b.ID_Predmeta = ? and b.Tip_Aktivnosti = 1 Order by b.Naziv_Aktivnosti DESC"));
...
void extraGrades(const gui::Image* pImage, td::INT4 SubjectID, td::INT4 type)
{
	dp::IDatabase* pDB = dp::getMainDatabase();
	...
	//#endif
		//Body 
	{
		dp::IDataSet* pDP(pDB->createDataSet("SELECT a.ID_Korisnika, a.ID_Aktivnosti, b.Naziv_Aktivnosti, c.Indeks, c.Ime, c.Prezime, d.Ocjena as Procenat FROM PolazniciAktivnosti a JOIN Aktivnosti b ON a.ID_Aktivnosti = b.ID_Aktivnosti JOIN Korisnici c ON a.ID_Korisnika = c.ID JOIN OcjeneLabZadace d ON b.ID_Aktivnosti = d.ID_Aktivnosti AND a.ID_Korisnika = d.ID_Korisnika WHERE b.ID_Predmeta = ? AND b.Tip_Aktivnosti = ? AND d.Ocjena IS NOT NULL"));
